(function ($) {
  ...
  $(function () {
      ...
      var SalesChart = new Chart(SalesChartCanvas, {
        ...
        data: {
          ...
          datasets: [{
            ...
            data: [48, 230, 47, 21, 33, 0, 0, 0, 0, 0, 0, 0],
            backgroundColor: '#98BDFF'
          },
          {
            label: 'Tổng đơn hàng',
            data: [40, 340, 55, 480, 17, 0, 0, 0, 0, 0, 0, 0],
    ...
    // Lấy dữ liệu từ API
    $.get('/admin/saleStats/getListOrder')
      .then(function (response) {
        var data = response;

    // Lấy các giá trị "deliveredOrders" và "totalOrders" từ mỗi đối tượng
    var deliveredOrdersData = data.map(function(item) {
      return item.deliveredOrders;
    });

    var totalOrdersData = data.map(function(item) {
      return item.totalOrders;
    });

    console.log(deliveredOrdersData);
    // Cập nhật dữ liệu trong biểu đồ
    SalesChart.data.datasets[0].data = deliveredOrdersData;
    SalesChart.data.datasets[1].data = totalOrdersData;
